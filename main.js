...
function renderPackages(packages) {
    ...
    packages.forEach(pkg => {
        ...
        <div style="padding: 1rem;">
            <h5 style="font-size: 1.25rem; font-weight: bold;">${pkg.name}</h5>
            <h6 style="font-size: 1rem; font-weight: bold;color:#007bff;">$${pkg.price.toFixed(2)}</h6>
            <div style="height:200px;">
            <p style="font-size: 1rem;height 150px;">${pkg.description}</p>
            </div>
            <button style="font-size: 1rem; padding: 0.5rem 1rem; background-color: #007bff; color: #fff; border: none; border-radius: 0.25rem;" id="cart-button" onclick="addToCart(${pkg.id})">Add to Cart</button>
        </div>
    </div>
</div>
